...
public class KimiTranslationEngine extends BaseTranslationEngine {
    ...
    public void onStart() {
        ...
        temperature = preferences.getString(
    ...
    public String translate(String text, String sourceLanguage, String targetLanguage) throws IOException {
        if (apiKey == null || apiKey.isEmpty()) {
            throw new IOException(getContext().getString("{kimi_api_key_empty}"));
        }
        if (model == null || model.isEmpty()) {
            throw new IOException(getContext().getString("{kimi_model_empty}"));
        }
        ...
        return requestTranslation(apiKey, model, temperature, text, sourceLanguage, targetLanguage);
    }
    ...
    static String requestTranslation(
        ...
        String requestBody = buildRequestBody(model, temperature, text, sourceLanguage, targetLanguage);
        Request request = new Request.Builder()
                .url(KimiConstant.KIMI_API_URL)
    ...
    /**
     * 温度留空表示不发送该参数，跟随模型默认值
     */
    static Double parseTemperature(String rawTemperature) throws IOException {
        ...
        try {
            ...
            throw new IOException("Invalid temperature value: " + value);
        }
    }
